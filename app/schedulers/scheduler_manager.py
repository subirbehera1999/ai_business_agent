...
async def _run_with_lock(
    ...
    result = await run_job(job_name=job_name, job_fn=job_fn)
...
def _db_session():
    ...
    Wraps get_db_session() from app/database/db.py.
    Each job gets its own isolated session — sessions are never shared
    between jobs or between scheduler invocations.
    """
