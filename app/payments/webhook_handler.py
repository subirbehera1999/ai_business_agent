...
# ==============================================================================
# Module-level helper used directly by webhook_route.py
# ==============================================================================
...
async def verify_and_parse_webhook(
    ...
    """
    Standalone utility for routes that need signature verification
    without the full handler class.

    Reads the raw body, verifies the HMAC signature, and returns
    the parsed payload if valid.

    This function is an alternative entry point for simple route
    implementations that prefer function-based over class-based injection.

    Args:
        request:         FastAPI Request.
        razorpay_client: Injected RazorpayClient instance.

    Returns:
        tuple[is_valid, parsed_payload, error_message]
    ...
    try:
        ...
        return False, None, f"Failed to read request body: {exc}"
    ...
    signature = request.headers.get(_RAZORPAY_SIGNATURE_HEADER, "").strip()
    if not signature:
        return False, None, "Missing X-Razorpay-Signature header"
