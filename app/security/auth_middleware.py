...
# Tracks failed auth attempts per IP in memory.
...
def _is_ip_rate_limited(client_ip: str) -> bool:
    ...
    Uses an in-memory sliding window. Evicts timestamps older than
    _FAILED_ATTEMPT_WINDOW_SECONDS before checking the count.
    """
    now = time.monotonic()
    ...
    _failed_attempts[client_ip] = fresh
